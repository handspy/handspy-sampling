...
import pt.up.hs.sampling.config.KafkaProperties;
...
class SamplingKafkaResourceIT {
    ...
    void setup() {
        ...
        SamplingKafkaResource kafkaResource = new SamplingKafkaResource(kafkaProperties);
...
//    @Test
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/sampling-kafka/publish/topic-produce?message=value-produce"))
...
//    @Test
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/sampling-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
